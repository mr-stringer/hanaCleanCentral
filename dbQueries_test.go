...
func TestGetAlertCount(t *testing.T) {
	...
	tests := []struct {
		...
		{"Good01", args{1}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -1) LIMIT 1"},
		{"Good02", args{2}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -2) LIMIT 1"},
		{"Good03", args{4}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -4) LIMIT 1"},
		{"Good04", args{10}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -10) LIMIT 1"},
		{"Good05", args{28}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -28) LIMIT 1"},
		{"Good06", args{60}, "SELECT COUNT(SNAPSHOT_ID) AS COUNT FROM \"_SYS_STATISTICS\".\"STATISTICS_ALERTS_BASE\" WHERE ALERT_TIMESTAMP < ADD_DAYS(CURRENT_TIMESTAMP, -60) LIMIT 1"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
